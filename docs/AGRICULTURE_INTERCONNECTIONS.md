...
The AGX machines live in the canonical corpus, at
`RealityEngine_Machines/machines/domains/agriculture/`, and are maintained as a
corpus change rather than regenerated from this repo. The generators that first
emitted them — including the Yuma AGX051–AGX055 generator — wrote to
`RealityEngine_CI/examples/machines/`, the retired RealityEngine_AI layout. They
were removed rather than repointed, because `MACHINE_CONCEPT.md` §9 now governs
how a domain changes.
...
and never edited by hand (§9.1); that is what now removes unused coordinate
positions while preserving the intended output-to-input overlap topology. Adding
or removing AGX machines follows the order of operations in §9.4, §9.2 defines
what acceptance validates, and §9.3 states when the domain must extend the
regression corpus.
