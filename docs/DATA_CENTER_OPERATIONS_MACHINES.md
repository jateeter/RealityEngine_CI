...
`RealityEngine_Machines/machines/domains/data-center/`, and are maintained as a
...
and never edited by hand (§9.1); that is what now preserves existing
interconnections while removing unused coordinate positions. Adding or removing
machines follows the order of operations in §9.4, §9.2 defines what acceptance
validates, and §9.3 states when the domain must extend the regression corpus.
