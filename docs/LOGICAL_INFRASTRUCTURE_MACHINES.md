...
These machines live in the canonical corpus, under
`RealityEngine_Machines/machines/domains/digital-logic/` and
`machines/domains/ai-services/`, and are maintained as a corpus change rather
than regenerated from this repo. The generator that first emitted them wrote to
`RealityEngine_CI/examples/machines/`, the retired RealityEngine_AI layout, and
was removed rather than repointed, because `MACHINE_CONCEPT.md` §9 now governs
...
and never edited by hand (§9.1); that is what now preserves existing overlaps
while removing unused coordinate positions. Adding or removing machines follows
