...
|---|---|---|
| **G1** | Certification runs and passes on every merge to main | **In progress** — 4 of 5 live stages green |
| **G2** | Versions pinned across repos, reproducibly | Partial |
...
| MCP open service | **pending** | every known defect fixed and merged; awaiting run 31297685782 |
...
Missing: a release manifest pinning each repo to a **SHA** rather than a branch,
so a tagged MVP can be rebuilt exactly. The regression harness already records
per-repo provenance per run — that record is the natural seed for the manifest.
